...
import { getDeliveryOrders, deleteDeliveryOrderById } from 'apiSdk/delivery-orders';
import { DeliveryOrderInterface } from 'interfaces/delivery-order';

type ColumnType = ColumnDef<DeliveryOrderInterface, unknown>;
...
export function DeliveryOrderListPage(props: DeliveryOrderListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DeliveryOrderInterface>>(
    () => `/delivery-orders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteDeliveryOrderById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: DeliveryOrderInterface) => {
    if (hasAccess('delivery_order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/delivery-orders/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/delivery-orders/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('delivery_order', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/delivery-orders/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('delivery_order', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="delivery_order" />
        </Flex>
        {hasAccess('delivery_order', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/delivery-orders/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
